...
Section SectionFactory::createDefaultSection() {
    return Section("Default")
        ...
        .addParameter(SynthParameter("Sequencer")
                          .number(428)
                          .max(1)
                          .descriptions({ "OFF", "ON" }));
}
...
Section SectionFactory::createVoiceSection() {
    return Section("Voice")
        .addParameter(SynthParameter("Transpose").type(CENTER_128).number(241))
        ...
        .addParameter(SynthParameter("Glide").number(245).max(2).descriptions(
            { "Off", "Always", "Only Legato Notes" }
        ))
    ...
    return Section("Mixer")
        .addSubSection(
            Section("Output")
                ...
                .addParameter(
                    SynthParameter("Panning").number(247).type(CENTER_128)
                )
                .addParameter(
...
Section SectionFactory::createEffectsSection() {
    return Section("Effects")
        .addSubSection(
            Section("Main")
                .addParameter(SynthParameter("Bandwidth")
                                  .number(340)
                                  .max(7)
                                  .descriptions(
                                      { "Full", "20 kHz", "18 kHz", "16 kHz",
                                        "14 kHz", "12 kHz", "10 kHz", "8 kHz" }
                                  ))
                .addParameter(SynthParameter("FX Routing")
                                  .number(508)
                                  .max(2)
                                  .descriptions(
                                      { "Chor/Phas/AM/Gate/Dly",
                                        "Gate/Dly/Chor/Phas/AM",
                                        "Bypass effects" }
                                  ))
                .addParameter(SynthParameter("Bitcrusher depth").number(380))
                .addParameter(SynthParameter("Decimator depth").number(370))
                .addParameter(SynthParameter("Filter HI").number(320))
                .addParameter(SynthParameter("Filter LO").number(321))
        )
        .addSubSection(
            Section("Distortion")
                .addParameter(SynthParameter("On or Off")
                                  .number(350)
                                  .max(1)
                                  .descriptions({ "OFF", "ON" }))
                .addParameter(
                    SynthParameter("Type").number(354).max(3).descriptions(
                        { "Hard clip", "Soft clip", "Tube 12AX", "Tube DSL" }
                    )
                )
                .addParameter(SynthParameter("Gain PRE").number(351))
                .addParameter(SynthParameter("Gain POST").number(352))
                .addParameter(SynthParameter("Filter POST")
                                  .number(353)
                                  .max(7)
                                  .descriptions(
                                      { "Full", "20 kHz", "18 kHz", "16 kHz",
        ...
        .addSubSection(
            Section("Chorus/Flanger")
                ...
                .addParameter(
                    SynthParameter("Mode").number(362).max(3).descriptions(
                        { "Chorus Long", "Chorus Short", "Flanger Long",
                          "Flanger Short" }
                    )
                )
                ...
                .addParameter(
                    SynthParameter("LR Phase").number(366).type(CENTER_128)
                )
        )
        .addSubSection(
            Section("Phaser")
                ...
                .addParameter(
                    SynthParameter("Mode").number(312).max(2).descriptions(
                        { "Mono", "Stereo", "Cross" }
                    )
                )
                ...
                .addParameter(
                    SynthParameter("Stages").number(317).min(4).max(12)
                )
                .addParameter(SynthParameter("Feedback").number(315))
                .addParameter(
                    SynthParameter("LR Phase").number(318).type(CENTER_128)
                )
        )
        .addSubSection(
            Section("Delay")
                ...
                .addParameter(
                    SynthParameter("Mode").number(302).max(2).descriptions(
                        { "Mono", "Stereo", "Cross" }
                    )
                )
                ...
                .addParameter(
                    SynthParameter("Smear").number(291).max(7).descriptions(
                        { "None", "16-bit", "15-bit", "14-bit", "13-bit",
                          "12-bit", "11-bit", "10-bit" }
                    )
                )
                .addParameter(
                    SynthParameter("2x").number(291).max(1).descriptions(
                        { "Off", "2x Delay" }
                    )
                )
        )
        .addSubSection(
            Section("Reverb")
                ...
                .addParameter(
                    SynthParameter("Mode").number(392).max(1).descriptions(
                        { "Plate", "Hall" }
                    )
                )
        ...
        .addSubSection(
            Section("Gate")
                .addParameter(SynthParameter("On or Off")
                                  ...
                                  .descriptions({ "OFF", "ON" }))
                .addParameter(
                    SynthParameter("Curve").number(386).max(1).descriptions(
                        { "S-Shape 1", "S-Shape 2" }
                    )
                )
...
Section SectionFactory::createArpSection() {
    return Section("ARP")
        .addParameter(SynthParameter("MODE").number(450).max(5).descriptions(
            { "Off", "Up", "Down", "Up/Down", "As played", "Random" }
        ))
        .addParameter(
            SynthParameter("Tempo").number(451)
            //                            .min(50) // TODO: Also allow 0 to
            //                            follow incoming MIDI clock tempo
        )
...
Section SectionFactory::createOscillatorSection() {
    return Section("Oscillators")
        ...
        .addParameter(SynthParameter("On or Off")
                          ...
                          .max(1)
                          .descriptions({ "OFF", "ON" }))
        .addParameter(SynthParameter("Mode")
                          .type(BITWISE)
                          .number(6)
                          .bitNumbers({ 0, 1, 2, 3 })
        ...
        .addParameter(SynthParameter("Waveform")
                          .numbers({ 11, 12, 13, 14 })
                          .max(8)
                          .descriptions(
                              { "Saw up", "Saw down", "Square", "Triangle",
                                "Sine", "Noise", "Sawstack x3 (stereo)",
        ...
        .addParameter(SynthParameter("Pulse Width")
                          .type(CENTER_128)
                          .numbers({ 15, 16, 17, 18 }))
        .addParameter(SynthParameter("Transpose")
                          .type(CENTER_128)
                          .numbers({ 19, 20, 21, 22 }))
        .addParameter(SynthParameter("Detune")
                          .type(CENTER_128)
                          .numbers({ 23, 24, 25, 26 }))
        .addParameter(
            SynthParameter("Sawstack Detune").numbers({ 285, 286, 287, 288 })
        )
        .addParameter(SynthParameter("Drift").numbers({ 260, 261, 262, 263 }))
        .addParameter(
            SynthParameter("Key breakpoint").numbers({ 43, 44, 45, 46 })
        )
        ...
        .addParameter(
            SynthParameter("Velocity sens.").numbers({ 39, 40, 41, 42 })
        )
...
Section SectionFactory::createEnvelopeSection() {
    return Section("Envelopes")
        ...
        .addParameter(
            SynthParameter("R5: Release 2").numbers({ 137, 136, 135 })
        )
        ...
        .addParameter(SynthParameter(""))
        .addParameter(
            SynthParameter("R4: Release 1").numbers({ 132, 131, 130 })
        )
        .addParameter(SynthParameter("Delay").numbers({ 112, 111, 110 }))

        .addParameter(
            SynthParameter("L0: Start").numbers({ 82, 81, 80 }).type(CENTER_128)
        )
        .addParameter(SynthParameter("L4: Release 1")
                          .numbers({ 102, 101, 100 })
                          .type(CENTER_128))
        .addParameter(SynthParameter("L1: Attack")
                          .numbers({ 88, 86, 85 })
                          .type(CENTER_128))
        .addParameter(SynthParameter("L5: Release 2")
                          .numbers({ 107, 106, 105 })
                          .type(CENTER_128))
        .addParameter(
            SynthParameter("L2: Decay").numbers({ 92, 91, 90 }).type(CENTER_128)
        )
        .addParameter(SynthParameter(""))
        .addParameter(SynthParameter("L3: Sustain")
                          .numbers({ 97, 96, 95 })
                          .type(CENTER_128))
        .addParameter(SynthParameter(""))

        .addParameter(SynthParameter("EG Loop")
                          .type(BITWISE)
                          .number(145)
                          .bitNumbers({ 0, 1, 6 })
                          .descriptions({ "Normal", "Loop" }))
        .addParameter(SynthParameter(""))
        .addParameter(SynthParameter("EG Loop Segment")
                          .type(BITWISE)
                          .number(146)
                          .bitNumbers({ 0, 1, 6 })
                          .descriptions({ "L0: Attack", "L5: Decay" }))
        .addParameter(SynthParameter(""))
        .addParameter(SynthParameter("EG Restart from")
                          .type(BITWISE)
                          .number(147)
                          .bitNumbers({ 0, 1, 6 })
                          .descriptions({ "L5: Decay", "L0: Start" }))
        .addParameter(SynthParameter(""))
        .addParameter(SynthParameter("Range").numbers({ 148, -1, -1 }).max(10))
        .addParameter(
            SynthParameter("Velocity").numbers({ 149, -1, -1 }).max(10)
        );
}
...
Section SectionFactory::createLFOSection() {
    return Section("LFO")
        .addSubSection(
            Section("LFO1")
                .addParameter(
                    SynthParameter("Wave").number(160).max(9).descriptions(
                ...
                .addParameter(SynthParameter("Sync").number(162).descriptions(
        ...
        .addSubSection(
            Section("LFO2")
                .addParameter(
                    SynthParameter("Wave").number(170).max(9).descriptions(
                        { "Triangle", "Square", "Saw Up", "Saw Down", "Sine",
                          "Sin( x ) + Sin( 2x )", "Sin( x ) + Sin( 3x )",
                          "Sin( x ) ^ 3", "Guitar", "S & H" }
                    )
                ...
                .addParameter(SynthParameter("Sync").number(172).descriptions(
                    { "Single, Free running", "Single, Key sync",
                      "Multi, Free running", "Multi, Key sync" }
                ))
...
Section SectionFactory::createFilterSection() {
    return Section("Filters")
        ...
        .addSubSection(
            Section("Settings")
                .addParameter(
                    SynthParameter("Type").number(71).max(21).descriptions(
                        { "Bypass",   "LPF 1P",   "LPF 2P",   "LPF 3P",
                          "LPF 4P",   "HPF 1P",   "HPF 2P",   "HPF 3P",
                          "HPF 4P",   "BPF 2P",   "BPF 4P",   "BRF 2P",
                          "BRF 4P",   "LP -> LP", "LP -> BP", "LP -> HP",
                          "LP // LP", "LP // BP", "LP // HP", "BP // BP",
                          "BP // HP", "HP // HP" }
                    )
                )
                .addParameter(
                    SynthParameter("Velocity").type(CENTER_128).number(73)
                )
                .addParameter(
                    SynthParameter("KBD Track").type(CENTER_128).number(74)
                )
                .addParameter(
                    SynthParameter("EG depth").type(CENTER_128).number(75)
                )
                .addParameter(
                    SynthParameter("EG Velocity").type(CENTER_128).number(76)
                )
                .addParameter(
                    SynthParameter("Velocity RESO").type(CENTER_128).number(276)
                )
                .addParameter(SynthParameter("KBD Track RESO")
                                  .type(CENTER_128)
                                  .number(277))
                .addParameter(SynthParameter("Drive").number(275).max(7))
                .addParameter(
                    SynthParameter("Routing").number(278).max(1).descriptions(
                        { "Standard", "LR Mode" }
                    )
                )
        );
...
Section SectionFactory::createExternalControlsSection() {
    return Section("External Ctrls")
        .addSubSection(
            Section("Pitch")
                .addParameter(
                    SynthParameter("Aftertouch").number(200).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Breath").number(201).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Foot").number(202).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Temperature").number(220).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("CV-1").number(221).type(CENTER_128)

                )
                .addParameter(
                    SynthParameter("CV-2").number(222).type(CENTER_128)
        ...
        .addSubSection(
            Section("Pulse Width")
                .addParameter(
                    SynthParameter("Aftertouch").number(204).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Wheel").number(205).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Breath").number(206).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Foot").number(207).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Temperature").number(223).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("CV-1").number(224).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("CV-2").number(225).type(CENTER_128)
                )
        )
        .addSubSection(
            Section("Cutoff")
                .addParameter(
                    SynthParameter("Aftertouch").number(208).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Wheel").number(209).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Breath").number(210).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Foot").number(211).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("Temperature").number(226).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("CV-1").number(227).type(CENTER_128)
                )
                .addParameter(
                    SynthParameter("CV-2").number(228).type(CENTER_128)
                )
        )
        .addSubSection(
            Section("Volume")
...
Section SectionFactory::createPerformanceControlsSection() {
    return Section("Performance Ctrls")
        .addParameter(
            SynthParameter("Midi CC 70").performanceControlType(400, 401)
        )
        .addParameter(
            SynthParameter("Midi CC 74").performanceControlType(408, 409)
        )
        .addParameter(
            SynthParameter("Midi CC 71").performanceControlType(402, 403)
        )
        .addParameter(
            SynthParameter("Midi CC 75").performanceControlType(410, 411)
        )
        .addParameter(
            SynthParameter("Midi CC 72").performanceControlType(404, 405)
        )
        .addParameter(
            SynthParameter("Midi CC 76").performanceControlType(412, 413)
        )
        .addParameter(
            SynthParameter("Midi CC 73").performanceControlType(406, 407)
        )
        .addParameter(
            SynthParameter("Midi CC 77").performanceControlType(414, 415)
        );
}
...
Section SectionFactory::createSequencerSection() {
    return Section("Sequencer")
        ...
        .addSubSection(
            Section("Steps")
                .addParameter(
                    SynthParameter("Step 1").number(434).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 5").number(438).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 2").number(435).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 6").number(439).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 3").number(436).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 7").number(440).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 4").number(437).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 8").number(441).type(MIDI_NOTE)
                )

                .addParameter(
                    SynthParameter("Step 9").number(442).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 13").number(446).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 10").number(443).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 14").number(447).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 11").number(444).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 15").number(448).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 12").number(445).type(MIDI_NOTE)
                )
                .addParameter(
                    SynthParameter("Step 16").number(449).type(MIDI_NOTE)
                )
